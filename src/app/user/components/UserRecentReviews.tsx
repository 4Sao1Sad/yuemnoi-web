...
export interface UserRecentReviewProp { id: string, reviewer_name: string, surname: string, reviewer_id: number, score: number, description: string }
...
export default function UserRecentReview({ data }: { data: UserRecentReviewProp[] }) {
    return <div className="h-fit flex flex-1 flex-col space-y-4">
        {data.map(({ id, reviewer_name: name, surname, reviewer_id, score, description }) => {
